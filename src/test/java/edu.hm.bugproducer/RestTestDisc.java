...
public class RestTestDisc {
    ...
    private void addOneBook(String title, String name, String barcode, int fsk) throws IOException {
        HttpClient client = HttpClientBuilder.create().build();

        JSONObject disc = new JSONObject();
        disc.put("title", title);
        disc.put("director", name);
        disc.put("barcode", barcode);
        disc.put("fsk", fsk);
